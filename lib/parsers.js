...
function diagnoseResponse(html) {
  ...
  var flags = [];
  if (!h) flags.push('EMPTY_BODY');
  if (h.length < 1500) flags.push('SUSPICIOUSLY_SHORT');
  if (/cf-browser-verification|cf_chl|Checking your browser|Just a moment|__cf_bm/i.test(h)) flags.push('CLOUDFLARE_CHALLENGE');
  if (/captcha|recaptcha|hcaptcha/i.test(h)) flags.push('CAPTCHA');
  if (/\/registracija|\/najava|Најави се|Регистрирај се/i.test(h) && !/\/kompanija\//i.test(h)) flags.push('POSSIBLE_LOGIN_WALL');
...
function parseProfile(html) {
  ...
  var phones = uniqBy(
    [].concat(telHrefs, telBlock, telFallback ? [telFallback] : [])
      .map(cleanPhone)
      .filter(isPhone),
    phoneKey
  );
  ...
  var emails = uniqBy(
    [].concat(mailHrefs, mailBlock, mailFallback ? [mailFallback] : [])
...
export {
  ...
  buildSearchUrl, diagnoseResponse,
