 ...
 */
function buildSearchUrl(regionCode, page, empFrom, empTo, townCode) {
  var from = empFrom == null ? 1 : empFrom;
  var to = empTo == null ? 5 : empTo;
  // c= is the town filter. Narrowing to one town keeps each search under the
  // site's ~60-result display ceiling, which region-wide searches blow past.
  var town = townCode == null ? '' : String(townCode);
  var url = CW_BASE + '/prebaruvanje'
    + '?cr=MKD&n=&mv=&r=' + regionCode + '&c=' + town + '&cp='
    // Empty date fields, present in the browser's own URL. They look inert, but
    // this is an ASP.NET model binder and absent is not the same as empty — a
    // live crawl went unproductive past page 3 without them.
    + '&FromDateAlt=&FromDate=&ToDateAlt=&ToDate='
    + '&at=&area=&subarea=&sbjact=t'
...
function diagnoseResponse(html) {
  ...
  // A challenge or login interstitial is a SMALL page served instead of the
  // site. A full-size page is the real site even when it carries zero results —
  // its ordinary chrome includes a login link and the reCAPTCHA script, and
  // treating those as a block turns an empty search into a fake outage.
  ...
  if (cfMarker) flags.push(looksReal ? 'CF_MARKER_PRESENT_IGNORED' : 'CLOUDFLARE_CHALLENGE');

  var captchaMarker = /captcha|recaptcha|hcaptcha/i.test(h);
  if (captchaMarker) flags.push(looksReal ? 'CAPTCHA_SCRIPT_PRESENT_IGNORED' : 'CAPTCHA');

  if (/\/registracija|\/najava|Најави се|Регистрирај се/i.test(h) && !looksReal) flags.push('POSSIBLE_LOGIN_WALL');

  // The ordinary "this filter matched nothing" page. Not an error.
  if (looksReal && !hasCompanyLinks) flags.push('NO_RESULTS_ON_PAGE');
